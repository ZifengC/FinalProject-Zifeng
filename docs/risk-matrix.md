...
This assessment converts the existing governance packet, monitoring results,
...
| R008 | User prompts, traces, or caches contain PII or confidential content | Privacy | 3 | 5 | 15 HIGH | Local prototype deployment, no intentional sensitive corpus, governance warnings | 2 | 4 | 8 MEDIUM | Mitigate | Security owner | Sensitive prompt report, logging review failure, retention-policy gap |
...
| R011 | System is reused for high-impact decisions outside the intended low-risk scope | Compliance / Accountability | 2 | 5 | 10 HIGH | System card, governance packet, model limitations, out-of-scope statements | 2 | 4 | 8 MEDIUM | Avoid | Governance owner | New deployment request, external user onboarding, policy review |
...
## LLM/RAG-Specific Risks

The project includes several LLM/RAG-specific risks that are central to the
system design:
...
This is enough to classify unsupported-answer risk as operationally
...
The project is local and prototype-scoped, which lowers exposure, but the
...
| Transfer | None used in this prototype |
...
Decision:
`PROCEED WITH CONDITIONS` for local prototype use.
